          ...
          <div v-if="selectedImagePaths.length">
            ...
            >
              <div v-if="index === 0" class="image-title">
                <h3 class="title">Protein Celltype</h3>
                <hr class="cell-line" />
              </div>
              <div v-if="index === 1" class="image-title">
                <h3 class="title">Protein Neighborhood</h3>
...
export default {
  ...
  methods: {
    async loadExcelData() {
      try {
        const response = await axios.get("datasets.xlsx", {
    ...
    async handleNodeClick(region) {
      ...
      if (path.length === 4) {
        ...
        const imagePathBase = `/multi_inte/${technology}/${dataset}/${tissue}/${regionLabel}/`;
