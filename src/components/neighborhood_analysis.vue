      ...
      <el-container>
        ...
        <el-main>
          <div v-if="selectedImagePaths.length">
            <div v-for="(path, index) in selectedImagePaths" :key="index">
              <img :src="path" style="width: 100%; max-width: 1350px; margin-bottom: 20px;" @error="imageLoadError" />
            </div>
          </div>
        </el-main>
      </el-container>
...
export default {
  ...
  methods: {
    ...
    handleNodeClick(region) {
      const path = this.getNodePath(region);
      console.log('Node path:', path); // 调试信息
      if (path.length === 4) {
        const [technology, dataset, tissue, region] = path;
        const imagePathBase = `/datasets/${technology}/${dataset}/${tissue}/${region}/`;
