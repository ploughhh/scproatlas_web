    ...
    >
      <div style="width: 1600px; margin-left: 250px">
        <el-table :data="tableData" stripe :row-style="{ height: '65px' }" width="100%">
          ...
          <el-table-column label="Whole Pipeline" width="200">
            <template #default="scope">
              <router-link
                :to="{
                  name: 'wholePipeline',
                  query: {
                    Technology: scope.row.Technology,
                    Dataset: scope.row.Dataset,
                    Tissue: scope.row.Tissue,
...
export default {
  ...
  methods: {
    loadTableData() {
      axios
        .get("/datasets/datasets.xlsx", { responseType: "arraybuffer" })
        .then((response) => {
